...
// תמונות מקומיות (img/, ובציר הזמן גם תמונות הקבלות ב-assets/) מוטמעות כ-data URI כדי לעבור את ה-CSP של ה-Artifact
const inlineImages = (html, dirs = ["img"]) => {
...
// מסמן לדפים שהם רצים בתוך Artifact (למשל: אין קישור ל-PDF של הקבלה — הדפדפן חוסם שם הורדות)
const artifactFlag = "<script>window.TRIP_ARTIFACT = true;</script>\n";
